...
//        return object.toJSONString();
    ...
    public  DataList findMaterialStore(Integer materialId,Double count,DataRow rowList){
        ...
        if(type.getId()==1||type.getId()==16||type.getId()==17||type.getId()==10||type.getId()==12||type.getId()==13||type.getId()==15){
            ...
            while (deCount>0&&index<storeList.size()){
                ...
                map.put("productName",rowList.get("productName"));
                map.put("projectId",rowList.get("projectId"));
                map.put("projectName",rowList.get("projectName"));
                map.put("buildingId",rowList.get("buildingId"));
                map.put("buildingName",rowList.get("buildingName"));
                map.put("buildingpositionId",rowList.get("buildingpositionId"));
                map.put("buildingpositionName",rowList.get("buildingpositionName"));
    ...
    public boolean materialRequisition(String s,String s1,Integer operator){
        JSONArray jsonArray =JSONArray.parseArray(s);
        for(int i=0;i<jsonArray.size();i++){
