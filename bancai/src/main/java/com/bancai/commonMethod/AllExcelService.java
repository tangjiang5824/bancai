...
//			String inventoryUnit = dataRow.get("单位") + "";
			...
			String partNo = (dataRow.get("品号") + "").trim().toUpperCase();
...
//			String inventoryUnit = dataRow.get("单位") + "";
	 ...
	 */
	@Transactional
	public UploadDataResult uploadProductExcelBasicInfo(InputStream inputStream) throws IOException {
		UploadDataResult result = new UploadDataResult();
		Excel excel = new Excel(inputStream);
		DataList excelList = excel.readExcelContent();
		Iterator it = excelList.iterator();
