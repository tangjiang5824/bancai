...
public class Upload_Data_Service extends BaseService {
	...
	@Transactional
	public void addData(int proNum,String ProName,String scale,String respo,String respoNum,int count,double cost,String location){
		
		
		jo.update("insert into oldpanelstore(品号, 品名,规格,库存单位,仓库编号,数量,成本,存放位置) values(?,?,?,?,?,?,?,?)",
				proNum,ProName,scale,respo,respoNum,count,cost,location);
		//return true;
	}
}